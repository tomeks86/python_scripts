...
for i in range(Na):
	for j in range(Nb):
		...
		add_molecule(mol,warstwa,XYZ,np.array([x+(rdm()-.5)*dX,y+(rdm()-.5)*dY,Z+(rdm()-.5)+2]))
if dNa:
	x+=dX
	dY=Y/r
	for i in range(r):
	...
	for i in range(r):
		...
		XYZ=np.dot(rot_x(2.*np.pi*rdm()),np.dot(rot_y(2.*np.pi*rdm()),rot_z(2.*np.pi*rdm())))
		add_molecule(mol,warstwa,XYZ,np.array([x+(rdm()-.5)*dX,y+(rdm()-.5)*dY,Z+(rdm()-.5)+2]))
set_id(warstwa,etykieta)

translate_mol(warstwa,-CM(warstwa))
translate_mol(warstwa,[0,0,Z])
